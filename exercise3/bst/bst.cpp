...
bool BST<Data>::operator == (const BST & other) const noexcept{
    ...
    bool equal = true;
    this->InOrderTraverse([&other, &equal](const Data & data){
...
void BST<Data>::RemoveMin(){
    ...
    Remove(FindPointerToMin(root)->Element());
...
void BST<Data>::RemoveMax(){
    ...
    Remove(FindPointerToMax(root)->Element());
}
...
const Data & BST<Data>::Predecessor(const Data & elem) const{
    ...
    const typename BST<Data>::NodeLnk * const * pred = &FindPointerToPredecessor(root, elem);
    if(pred != nullptr)
        return (*pred)->element;
    else
...
const typename BST<Data>::NodeLnk* const& BST<Data>::FindPointerToPredecessor(const typename BST<Data>::NodeLnk* const & node, const Data & elem) const noexcept {
  const NodeLnk * const * ret = nullptr;
  const NodeLnk * const * current = &node;
  ...
  if ((*current) != nullptr && (*current)->left!= nullptr)
    ret = &FindPointerToMax((*current)->left);
  return *ret;
}
...
const Data & BST<Data>::Successor(const Data & elem) const{
    ...
    const NodeLnk * const * succ = &FindPointerToSuccessor(root, elem);
    if(succ != nullptr)
        return (*succ)->element;
    else
...
const typename BST<Data>::NodeLnk* const& BST<Data>::FindPointerToSuccessor(const typename BST<Data>::NodeLnk* const& node, const Data & elem) const noexcept {
  ...
  const typename BST<Data>::NodeLnk *const *current = &node;
  while ((*current) != nullptr && (*current)->element != elem) {
    if ((*current)->element < elem)
      current = &(*current)->right;
    else if ((*current)->element > elem) {
  ...
  if ((*current) != nullptr && (*current)->right != nullptr)
    ret = &FindPointerToMin((*current)->right);
  return *ret;
}
...
typename BST<Data>::NodeLnk *& BST<Data>::FindPointerToSuccessor(typename BST<Data>::NodeLnk *& node, const Data & elem) noexcept {
  ...
  typename BST<Data>::NodeLnk ** current = &node;
  while ((*current) != nullptr && (*current)->element != elem) {
    if ((*current)->element < elem)
      current = &(*current)->right;
    else if ((*current)->element > elem) {
